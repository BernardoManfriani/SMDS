...
glm_mod7 <- glm(nuovi_positivi ~  (ricoverati_con_sintomi_prev + nuovi_tamponi_pcr_prev) * color_prev + data, data = df, family=poisson(link = "log"))
plot(glm_mod7)
...
### ricoverati_con_sintomi + nuovi_taponi + color + data

### (ricoverati_con_sintomi + nuovi_tamponi)*color + data + google
```{r gam_mod1, echo=FALSE}
library(mgcv)
gam_mod1 <- gam(nuovi_positivi ~  s(ricoverati_con_sintomi, bs="cs") + s(nuovi_tamponi_pcr, bs="cs") + color + data , data = df )
...
ggplot() +
  geom_point(data = df, aes(x = data, y = nuovi_positivi, color="nuovi positivi", group = 1),col=c("#fc6b03","#cfcaca","#f2d729","#b3190b")[unclass(as.factor(df$color))])+ 
geom_line(data = df, aes(x = data, y = predict(gam_mod1), color = "model 4", group = 1)) +
...
### (ricoverati_con_sintomi + nuovi_tamponi)*color + data

```{r gam_mod2, echo=FALSE}
library(mgcv)
gam_mod2 <- gam(nuovi_positivi ~   (ricoverati_con_sintomi + nuovi_tamponi_pcr)*color + data , data = df )
...
```{r gam predictions2, echo=FALSE, message=FALSE, warning=FALSE}
...
```
## Prev vs Current data


```{r prev vs current, echo = FALSE}
glm_prev <- glm(nuovi_positivi ~ (ricoverati_con_sintomi_prev + nuovi_tamponi_pcr_prev)*color_prev, data = df, family = poisson(link = "log") )
glm_current <- glm(nuovi_positivi ~ (ricoverati_con_sintomi + nuovi_tamponi_pcr)*color, data = df, family=poisson(link = "log"))
...
# Conclusion : CURRENT DATA ARE BETTER !!


## LOESS
```{r LOESS}
library(mgcv)
loess_mod1 <- loess(nuovi_positivi ~ ricoverati_con_sintomi + nuovi_tamponi_pcr  , data = df )
