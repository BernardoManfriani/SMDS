...
for(x in 8:nrow(df_extended)) {
  df_extended$nuovi_tamponi_pcr[x] <- df_extended$tamponi_test_molecolare[x] - df_extended$tamponi_test_molecolare[x-1] # the number of new swabs carried out
...
plot(nuovi_positivi ~ nuovi_tamponi_pcr, data = df, pch =16, xlab = "PCR test", ylab = "Daily number of new positives")
...
geom_line(data = df_extended, aes(x = data, y = predict(glm_mod6, newdata=df_extended, type="response"), color = "model 4", group = 1)) +
...
geom_line(data = df_extended, aes(x = data, y = predict(glm_mod7, newdata=df_extended, type="response"), color = "model 4", group = 1)) +
   geom_ribbon(data=df_extended, aes(x=data, ymin=glm_mod7_pred$fit - glm_mod7_pred$se.fit, ymax=glm_mod7_pred$fit + glm_mod7_pred$se.fit),alpha=0.2)+
...
geom_line(data = df_extended, aes(x = data, y = predict(gam_mod1, newdata=df_extended, type="response"), color = "model 4", group = 1)) +
  geom_ribbon(data = df_extended, aes(x= data, ymin=-gam_mod_1_pred$se.fit + gam_mod_1_pred$fit, ymax=gam_mod_1_pred$se.fit + gam_mod_1_pred$fit), alpha=0.1)+
