...
residuals <- df$nuovi_positivi - pred
#res <- resid(nglmod5)
ggplot() +
  geom_point( aes(x = pred, y = residuals, group = 1),col="#2783c4") +
  geom_smooth(aes(x = pred, y = residuals), col ="#45a377") +
  geom_line(aes(x= pred, y = 0), lty=2) +
...
library(mgcv)
gam_mod1 <- gam(nuovi_positivi ~  s(ricoverati_con_sintomi) + s(terapia_intensiva) + s(ricoverati_con_sintomi_prev) + s(nuovi_tamponi_pcr) , data = df )
...
knots <- quantile(df$ricoverati_con_sintomi_prev)[2:4]
fit.spline <- lm (nuovi_positivi ~ bs(ricoverati_con_sintomi_prev, knots = knots), data = df)
summary(fit.spline)

ggplot() +
  geom_point(data = df_extended,aes(data, nuovi_positivi), col=col_zone[unclass(as.factor(df_extended$color))]) +
  stat_smooth(data = df_extended,aes(data, nuovi_positivi), method = lm, formula = y ~ splines::bs(x, df = 10), col="#31729e")
