...
library(mgcv)
gam_mod1 <- gam(nuovi_positivi ~  s(ricoverati_con_sintomi*color)  , data = df,family=Gamma(link=log) )
...
ggplot() +
  ...
  geom_line(data = df, aes(x = data, y = new_color, group = 1), size=0.75,col=c("#fc6b03","#cfcaca","#f2d729","#b3190b")[unclass(as.factor(df$color))]) +
...
p<- ggplot() +
  ...
  geom_line(data = df_extended, aes(x = data, y = predict(glm_mod, newdata = df_extended, type="response"), color = "model 1", group = 1)) + labs(x = "Time", y = "New positives") +
  scale_x_date(date_breaks = '5 days',date_labels = '%Y-%m-%d') +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
...
g <- ggplot(data = df_extended)+
  ...
  geom_line(data = df_extended, aes(x = data, y = predict(glm_mod,newdata = df_extended, type="response"), color = "model 1", group = 1)) +
  scale_x_date(limits = c(d1, d2)) + scale_y_continuous(limits = c(400, 1000)) + theme_bw()+
  theme(
...
p
d3 <- df_extended$data[80]
d4 <- df_extended$data[123]
