...
MAPE(predict(gam_mod1, type="response", newdata = df_extended[124:137,]), df_extended$nuovi_positivi[124:137])
...
library("Metrics")
#lm
1 - smape(predict(lm_mod5,type="response", newdata= df_extended[124:137,]), df_extended$nuovi_positivi[124:137]) #  0.3325639

#glm poisson
1 - smape(predict(glm_mod5,type="response", newdata= df_extended[124:137,]), df_extended$nuovi_positivi[124:137]) # 0.3308621


#glm negative binomial
1 - smape(predict(nglmod5,type="response", newdata= df_extended[124:137,]), df_extended$nuovi_positivi[124:137]) # 0.3537849

#gam
1 - smape(predict(gam_mod1,type="response", newdata= df_extended[124:137,]), df_extended$nuovi_positivi[124:137]) # 0.4643648
...
ggplot() +
  ...
  geom_line(data = df, aes(x = data, y = new_color, group = 1), size=1.2,col=c("#fc6b03","#cfcaca","#f2d729","#b3190b")[unclass(as.factor(df$color))]) +
...
library("Metrics")
#lm
1 - smape(predict(lm_mod,type="response", newdata= df_extended[124:137,]), df_extended$nuovi_positivi[124:137]) #0.7969439

#glm poisson
1 - smape(predict(glm_mod,type="response", newdata= df_extended[124:137,]), df_extended$nuovi_positivi[124:137]) #0.8890304

#nglm
1 - smape(predict(nglm_mod,type="response", newdata= df_extended[124:137,]), df_extended$nuovi_positivi[124:137]) #0.8799145

#gam
1 - smape(predict(gam_mod,type="response", newdata= df_extended[124:137,]), df_extended$nuovi_positivi[124:137]) #0.8836843

```

Plot previous models andhighlight predictions:
```{r}
...
res
#ggsave(paste0( "images/lm_predictions.png"), width = 10, height = 8, dpi = 200)

res <- plot_predictions(glm_mod)
res
#ggsave(paste0( "images/glm_predictions.png"), width = 10, height = 8, dpi = 200)
...
#lm
1 - smape(predict(lm_mod_google,type="response", newdata = df_extended[124:137,]), df_extended$nuovi_positivi[124:137]) #0.7864553

#glm poisson
1 - smape(predict(glm_mod_google,type="response", newdata= df_extended[124:137,]), df_extended$nuovi_positivi[124:137]) 
#0.8775334
...
fit.spline <- lm (nuovi_positivi ~ bs(ricoverati_con_sintomi, knots = knots), data = df)
summary(fit.spline)



ggplot(df, aes(data, nuovi_positivi, color ="Spline model") ) +
  geom_point() +
  geom_point(col=col_zone[unclass(as.factor(df$color))])+
...
#ggsave(paste0( "images/spline.png"), width = 12, height = 5, dpi = 150)
