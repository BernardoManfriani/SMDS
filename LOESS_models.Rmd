...
```{r}
library(MLmetrics)
glm_google <- glm(nuovi_positivi ~ ricoverati_con_sintomi + as.numeric(data)+ var_station + nuovi_tamponi_pcr + new_color, data = df, family = "poisson")
...
```{r}
  ...
  labs(x = "Time", y = "New positives") +
  scale_x_date(date_breaks = '1 days', date_labels = '%Y-%m-%d') +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))+
  scale_color_brewer(palette = "Dark2")
