...
```{r}
nrw<-length(df[,1])
listlm<-c("lmod1","lmod2","lmod3","lmod4")
param<-c("ricoverati_con_sintomi","ricoverati_con_sintomi*color","(ricoverati_con_sintomi+terapia_intensiva)*color","(ricoverati_con_sintomi+terapia_intensiva+isolamento_domiciliare+nuovi_tamponi_pcr)*color")
num_var<-4
l<-colnames(df)
counter<-1
str<-"nuovi_positivi"
...
```{r}
nrw<-length(df[,1])
listlm<-c("pglmod1","pglmod2","pglmod3","pglmod4")
param<-c("ricoverati_con_sintomi","ricoverati_con_sintomi*color","(ricoverati_con_sintomi+terapia_intensiva)*color","(ricoverati_con_sintomi+terapia_intensiva+isolamento_domiciliare+nuovi_tamponi_pcr)*color")
num_var<-4
l<-colnames(df)
counter<-1
...
dfn<-data.frame("x"=x,"y"=y,"model"=m)
Gplot<-Gplot+geom_line( data=dfn,aes(x = x, y = y, color=model, group =model)) 
AIC(pglmod1,pglmod2,pglmod3,pglmod4)
Gplot  

## Model checking - Residual plots
pred <- predict(pglmod4,type="response")
y_res <- df$nuovi_positivi - pred
plot(pred, y_res, pch=16, col=4, xlab ="predictions", ylab = "residuals")
...
require("MASS")
nrw<-length(df[,1])
listlm<-c("nglmod1","nglmod2","nglmod3","nglmod4")
param<-c("ricoverati_con_sintomi","ricoverati_con_sintomi*color","(ricoverati_con_sintomi+terapia_intensiva)*color","(ricoverati_con_sintomi+terapia_intensiva+isolamento_domiciliare+nuovi_tamponi_pcr)*color")
num_var<-4
l<-colnames(df)
counter<-1
...
Gplot  
## Model checking - Residual plots
pred <- predict(nglmod4,type="response")
y_res <- df$nuovi_positivi - pred
plot(pred, y_res, pch=16, col=4, xlab ="predictions", ylab = "residuals")
